...
def contar_gatos(imagem_path: str, saida_path: Union[str, None] = None,
    ...
    # Filtra apenas classe 'cat'
    classes = boxes.cls.int().tolist()
    idx = [i for i, c in enumerate(classes) if names[int(c)] == "cat"]
    qtde = len(idx)

    # Desenha caixas apenas dos gatos
    img = cv2.imread(imagem_path)
    if img is None:
        raise FileNotFoundError(f"Não foi possível ler a imagem: {imagem_path}")
    ...
    for i in idx:
        ...
        cv2.putText(img, f"cat {conf_i:.2f}", (x1, max(20, y1 - 10)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

    # Título com contagem
    cv2.putText(img, f"Gatos: {qtde}", (10, 30),
...
def main():
    qtde = contar_gatos('cat_groups_entrada.jpg', 'cat_groups_saida.jpg', 0.4, 'yolov8n.pt')
    print(f"Contagem de gatos: {qtde}")
